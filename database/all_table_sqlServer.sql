   ...
   -- Kiểm tra và tạo Full-Text Catalog nếu chưa tồn tại
    IF NOT EXISTS (SELECT * FROM sys.fulltext_catalogs WHERE name = 'ftCatalog')
        CREATE FULLTEXT CATALOG ftCatalog AS DEFAULT;
    GO

    -- Tạo index duy nhất nếu chưa có (bắt buộc để làm key cho full-text index)
    IF NOT EXISTS (
        SELECT * FROM sys.indexes WHERE name = 'UX_posts_postId'
    )
        CREATE UNIQUE INDEX UX_posts_postId ON posts(postId);
    GO

    -- Tạo Full-Text Index duy nhất
    IF NOT EXISTS (
        SELECT * FROM sys.fulltext_indexes WHERE object_id = OBJECT_ID('posts')
    )
